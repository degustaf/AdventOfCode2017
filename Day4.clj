...
(= (validate "aa bb cc dd ee") true)
(= (validate "aa bb cc dd aa") false)
(= (validate "aa bb cc dd aaa") true)

(defn validate2 [line]
  (let [ls (map (comp frequencies seq) (clojure.string/split line #" "))]
    (= (count ls)
       (count (distinct ls)))))

(validate2 "abcde fghij")
(= (validate2 "abcde fghij") true)
(= (validate2 "abcde xyz ecdab") false)
(= (validate2 "a ab abc abd abf abj") true)
(= (validate2 "iiii oiii ooii oooi oooo") true)
(= (validate2 "oiii ioii iioi iiio") false)

(defn count-valid [f file]
    (apply + (map #(if (f %) 1 0) (clojure.string/split-lines file))))

(def passwords "vxjtwn vjnxtw sxibvv mmws wjvtxn icawnd rprh
...
(count-valid validate passwords)
(count-valid validate2 passwords)
