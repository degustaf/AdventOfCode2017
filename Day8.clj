(defn reg [data]
  (letfn [(max-val [m] (apply max (cons 0 (vals m))))
          (process-line [[register-map mval] line]
                        (let [[reg-mod chg chg-val _if reg-cmp op test-val] (clojure.string/split line #" ")
                              chg (if (= chg "inc") + -)
                              chg-val (Integer. chg-val)
                              test-val (Integer. test-val)
                              op (if (= op "!=") not= (resolve (symbol op)))
                              ret (if (op (get register-map reg-cmp 0) test-val)
                                      (assoc register-map reg-mod (chg (get register-map reg-mod 0) chg-val))
                                      register-map)]
                          (list ret (max mval (max-val ret)))))]
    (reduce process-line [{} 0] (clojure.string/split-lines data))))


(def data "sd dec 441 if k != 0
lp dec 419 if mxn >= 7
w inc -592 if icg >= -9
a dec -29 if q <= 9
kt dec 486 if ms != 8
